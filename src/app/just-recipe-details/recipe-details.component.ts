...
export class RecipeDetailsComponent implements OnInit,
  ...
  getTAID(id: number, idx: number) {
    let label = (id !== undefined
      ? id
      : 'newID');
    let count = (idx + 1);
    return label
      .toString()
      .concat('-rTA-', count.toString());
  }
  ...
  // Whenever the user needs to add a new `ingredient`, push an empty `ingredient`
  // object to the `ingredient` array on the `selectedRecipe`
  newIngredient() {
    // Check to see if the `ingredients` array exists before attempting to push an
    // `ingredient` to it
    ...
    this
      .recipe
      .ingredients
      .push(clone(this.rModel.ingredients[0]));
  }

  // Whenever the user needs to add a new `method`, push an empty `method` object
  // to the `method` array on the `selectedRecipe`
  newMethod() {
    // Check to see if the `method` array exists before attempting to push a
    // `method` to it
  ...
  // Whenever the user needs to add a new `tag`, push an empty `tag` object onto
  // the `tags` array on the `selectedRecipe`
  newTag() {
    // Check to see if the `tags` array exists before attempting to push a `tag` to
    // it
