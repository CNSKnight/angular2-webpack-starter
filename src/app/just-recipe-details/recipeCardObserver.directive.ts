...
export class RecipeCardObserver implements OnInit, DoCheck {
    ...
    checkElemHgt() {
        ...
        var hgt = this.$wrapper.height()
        if (hgt < 900 || this.lastHgt == hgt) return; // matches parent container min-height;
        this.details.setIframeWrapperHgt(hgt + 20);
        this.lastHgt = hgt;
    }
    ...
    // ngAfterViewInit() {
    //     console.log('BodyObserver.ngAfterViewInit')
    //     // meh - 
    //     if (!window || !window.parent || ! ("MutationObserver" in window)) return;

    // try {
    //     var details = window.parent.acap.ADMIN_TAPPADS.contUnitsMgr.details;
    //     var lastHgt;
    //     if (details) {
    //         details.onMuts = function (mutation, elem) {
    //             var hgt = elem.clientHeight;
    //             console.log(lastHgt, '|', hgt);
    ...
    //         var observer = this.Rx.DOM.fromMutationObserver(this.body.nativeElement, {
    //             childList: true,
    //             subTree: true
    //         });

    //         observer.subscribe(mutations => {
    //             mutations.forEach(mutation => details.onMuts(mutation, this.body.nativeElement));
    //         });
